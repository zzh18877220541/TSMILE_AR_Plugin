我在做一个AR插件。当前我完成了点击AR模型部件得到弹窗的部分。但是我想让我的弹窗随着模型一起移动。我现在的代码是点击对应部分部分，调用``showInfoPopup(partName: partName, targetPoint: location, modelTitle: partName)``函数，
...
```
 ...
 其中PopOver是一个继承UIView的类，代码附在最后。

现在的问题是。我如何在移动手机的过程中，让我的弹窗随着模型一起移动。
 我当前试过几种解决办法：
...
1. `transform`
   ...
   if let popOver = popOver {
       ...
       popOver.transform = CGAffineTransform(translationX: deltaX, y: deltaY)
   }
   ```
